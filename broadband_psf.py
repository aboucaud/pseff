...
class PSFcube(object):
    ...
    def to_broadband(self, sed, transmission, wl_step=1,
        ...
        transmission: `TelescopeThroughput`
        ...
        total_thruput = (sed(self.expanded_range, wlunit=self.wlunit) *
                         transmission(self.expanded_range, wlunit=self.wlunit))
    ...
    def downsample_detector(self, ds_factor=1):
        ...
        if ds_factor > 1:
            ...
            new_shape = tuple(np.array(psf_shape, dtype=int) // ds_factor)
...
def parse_args():
    ...
    parser.add_argument('sed', type=str, nargs='+',
...
def extract_config(configfile):
    """
    Extract typed information from config parser
    ...
    sed_wlunit = getattr(confparams.get('sed', 'wlunit').lower())

    sed_dict = dict(wlunit=sed_wlunit)

    # Broadband
    wl_step = confparams.getfloat('broadband', 'wl_step')
    down_fact = confparams.getfloat('broadband', 'down_fact')
...
def main():
    ...
    for psf_file in args.psf_cube_files:
        ...
        for sed_file in args.sed_files:
            ...
            psf.to_broadband(sed=sed, transmission=thruput,
                             wl_step=conf['wl_step'])
...
def test():
    ...
    BPZ = "/Users/aboucaud/work/Euclid/data/galaxy_spectra/bpz/{0}_B2004a.sed"
    ...
    P = PSFcube.from_fits(fitsfile, shape=(11, 512, 512), pixel_size=2, wlunit=u.um)
    S = FlatSED()
    # S = SED.from_ascii(BPZ.format(sedtype))
    T = TelescopeThroughput.from_file(transfile, cols=[0, 4], wlunit=u.nm)
    P.to_broadband(sed=S, transmission=T, wl_step=10)
    P.write_effective_psf(outfile)
    P.downsample_detector(4)
    P.write_effective_psf(outfile.replace('.fits', '_dwspl.fits'))

    dtime = int(time.time() - start_time)
    time_text = "Elapsed time: {0:02d}h{1:02d}m{2:02d}s"
